#!/usr/bin/env bash
#
# Stamps homebrew/dock-profiler.rb with a released version and its checksum,
# and with --push commits it to the Homebrew tap.
#
#   ./homebrew/publish-cask.sh              stamp the cask and print it
#   ./homebrew/publish-cask.sh --push       also push it to the tap
#
#   VERSION=1.1.0   the version to publish, default: package.json
#   ZIP=path/to.zip the archive to hash, default: the one in build/, and
...
#
# The cask in this repo is the source of truth. Everything except `version`
# and `sha256` is edited here by hand, and carried into the tap on the next
...
CASK="homebrew/dock-profiler.rb"
...
NAME="DockProfiler-$VERSION-macos-universal.zip"
...
if [ -z "${ZIP:-}" ]; then
  if [ -f "build/$NAME" ]; then
    ...
    curl -fsSL -o "$ZIP" \
      "https://github.com/estruyf/dock-profiler-macos/releases/download/v$VERSION/$NAME"
  fi
fi
...
if [ -n "${HOMEBREW_TAP_TOKEN:-}" ]; then
  # The usual CI shape: the token rides in the URL, Actions masks it in the
  # log, and the checkout is thrown away with the runner.
  git clone --depth 1 \
    "https://x-access-token:${HOMEBREW_TAP_TOKEN}@github.com/${TAP_REPO}.git" \
    "$WORK/tap"
else
  # Run locally with no token, this falls back to whatever `git`/`gh` already
  # has push access through — an SSH key or a `gh auth login` credential
  # helper for github.com.
  git clone --depth 1 "https://github.com/${TAP_REPO}.git" "$WORK/tap"
fi

mkdir -p "$WORK/tap/Casks"
cp "$CASK" "$WORK/tap/Casks/dock-profiler.rb"
...
git -C "$WORK/tap" add Casks/dock-profiler.rb
...
git -C "$WORK/tap" commit -m "dock-profiler $VERSION"
git -C "$WORK/tap" push
echo "==> Pushed dock-profiler $VERSION to $TAP_REPO"
